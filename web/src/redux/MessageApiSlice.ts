...
export const MessageApiSlice = apiSlice.injectEndpoints({
  ...
  >({
    query: (formData) => ({
      url: `media/uploadFileQueue?groupId=${formData.groupId}&userId=${formData.userId}&source=${formData.source}&location=${formData.location}&uploadBy=${formData.uploadBy}&private_chat_id=${formData.private_chat_id}&temp_id=${formData.temp_id}`,
      method: 'POST',
      body: formData.formData,
      formData: true,
      // headers: {
